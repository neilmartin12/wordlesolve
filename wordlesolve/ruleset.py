...
Rules are generated by adding outcomes to the RuleSet.
...
RuleSet methods:
    RuleSet.add_outcome(guess, outcome): add an outcome to this RuleSet
    RuleSet.is_match(word): check whether a word matches this RuleSet
    RuleSet.filter_matches(words): filter a list of words to retain only RuleSet matches
    RuleSet.score_words(words, matches): score a list of words according to this RuleSet
    RuleSet.alphabet(): get the alphabet color coded to show the status of each letter
...
class RuleSet(dict):
    ...
    def score_words(self, words: list[str], matches: list[str]) -> list[WordScore]:
        ...
        # Word frequency breaks tie for words in equal first position
        for index, word_score in enumerate(word_scores):
            if word_score.score != word_scores[0].score:
                break
